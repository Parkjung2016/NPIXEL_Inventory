...
public partial class InventoryData
{
    ...
    private ItemDataBase AddItemInternal(ItemDataBase itemData, bool invokeEvent = true)
    {
        ...
        if (stackableCandidate != null)
        {
            ...
            {
                existingStack.StackCount += stackableCandidate.StackCount == 0 ? 1 : stackableCandidate.StackCount;
        ...
        {
            newStackable.StackCount = stackableCandidate.StackCount == 0 ? 1 : stackableCandidate.StackCount;
    ...
    {
        return InternalSplitItem(itemData, splitCount, out _);
    }

    public bool InternalSplitItem(ItemDataBase itemData, int splitCount, out ItemDataBase newItemData)
    {
        newItemData = null;
        ...
        newItemData = itemData.DeepCopy();
        if (Application.isPlaying)
            AttributeInjector.Inject(newItemData, SceneManager.GetActiveScene().GetSceneContainer());
        IStackable newStackable = newItemData as IStackable;
        ...
        currentInventoryDataList[emptySlotIndex] = newItemData;
        _emptySlotPriorityQueue.TryRemove(emptySlotIndex);

        _stackableLookup.Add(newItemData.ItemID, emptySlotIndex, newStackable);
